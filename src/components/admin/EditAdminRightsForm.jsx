...
const EditAdminRightsForm = ({handleCloseOptions}) => {
  ...
  return (
    <form onSubmit={handleSubmit}>
              <h1 className='text-2xl mb-2'>Edit Administrator Permissions</h1>
              ...
              <label htmlFor='one' className='ml-2'>Create, delete and deactivate user accounts.</label>
              </div>
              
              <div className='mb-2 text-xl'>
              <input id="two" className='w-4 h-4 cursor-pointer' type="checkbox" checked={isChecked2} onChange={()=>handlecheckboxchange2()}></input>
              <label htmlFor='two' className='ml-2'>Create, edit and delete academic content.</label>

              </div>
              
              <div className='mb-2 text-xl'>
              <input id="three" className='w-4 h-4 cursor-pointer' type="checkbox" checked={isChecked3} onChange={()=>handlecheckboxchange3()}></input>
              <label htmlFor='three' className='ml-2'>Manage user roles and Permissions.</label>
              </div>
              <button className='py-1 px-5 bg-blue-600 hover:bg-blue-800 rounded-lg'>Save changes</button>
             
              <div onClick={handleCloseOptions} className='absolute w-12 h-8 top-2 right-4 rounded-full hover:text-blue-300 cursor-pointer'>
